...
class BasicClassifier:
    ...
    def train(
        ...
        for it in range(num_iters):
            ...
            random_select = np.random.choice(num_train,batch_size,replace=False)
            X_batch = X[random_select,:]
            y_batch = y[random_select]
            ...
            loss, dout = self.loss(X_batch, y_batch,reg) 
            W = self.W
            W = W - learning_rate*dout
...
class LogisticRegression(BasicClassifier):
    ...
    def predict(self, X):
        ...
        y_temp = sigmoid(X@self.W)
        y_pred = np.around(y_temp)
