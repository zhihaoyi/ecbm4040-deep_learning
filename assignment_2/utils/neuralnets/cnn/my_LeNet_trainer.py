...
class MyLeNet_trainer():
    ...
    """
    def __init__(self,X_train, y_train, X_val, y_val, epochs=14, batch_size=256, lr=1e-3):
    ...
    def batch_train_data(self, batch_size=256, shuffle=True):
        ...
        train_data.flip('v')
        train_data.brightness(1.3)
        train_data.add_noise(0.2,20)
    ...
    def train_epoch(self, epoch):
        ...
        for single_epoch in range(epoch):     
            train_ds = tf.data.Dataset.from_tensor_slices(next(self.train_data_next_batch)).batch(self.n_batches)
